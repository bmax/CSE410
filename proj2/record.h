...
struct argList {
  ...
  retVal * rVal;
  argList(){};
  argList(int si, int ei, string keyword, int tn, int ms, vector<int> c, retVal* r) : startIndex(si),endIndex(ei),keyword(keyword),threadno(tn),minSize(ms),criteria(c), rVal(r) {} ;
} ;
...
struct argListforRW {
  ...
  retVal * rVal;
  argListforRW(){};
  argListforRW(string file, retVal* r) : fileName(file), rVal(r) {} ;
} ;
